        ...
        <div class="modal-body">
          <form id="CreateCapsuleForm" @submit.prevent="createVault">
            ...
            <div class="form-outline mb-4">
              ...
              <input type="datetime-local" id="CreateCapsuleEndDateBoxID" class="form-control" name="openDate" v-model="form.eDate" required/>
            </div>
...
export default {
  ...
  methods: {
    async createVault() {
      //console.log("Comp", this.form)
      await this.capsuleState.createCapsule(this.form);
      const data = {}
      data['capCount'] = (this.usState.user)['capCount'] + 1
      //console.log(data)
      await this.usState.updateUser(data);
