...
function getPaper(){
  instance.get(`/system/newspaper/${router.currentRoute.value.params.id}/`).then(response => {
...
function savePaper(){
  ...
  instance.patch(`/system/newspaper/${router.currentRoute.value.params.id}/`, rest).then(response => {
        if (response.status === 200){
