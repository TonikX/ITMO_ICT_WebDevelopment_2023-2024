...
function getPaper(){
  instance.get(`/system/printing-newspapers/${router.currentRoute.value.params.id}/`).then(response => {
...
function savePaper(){
  ...
  instance.patch(`/system/printing-newspapers/${router.currentRoute.value.params.id}/`, rest).then(response => {
        if (response.status === 200){
