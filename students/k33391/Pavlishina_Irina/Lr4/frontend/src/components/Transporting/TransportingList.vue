...
function getNewspapers(){
  instance.get('/system/transporting/').then(response => {
...
function deleteNewspaper(id){
  instance.delete(`/system/transporting/${id}/`).then(response => {
...
<template>
  <div class="d-flex align-center flex-column ga-10">
    ...
    <v-card
        width="400"
        :title="`Заказ: ${newspaper.post_office_order}`"
        :subtitle="`Газета: ${newspaper.printing_newspaper}`"
