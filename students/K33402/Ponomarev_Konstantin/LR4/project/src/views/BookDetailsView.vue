              ...
              >
                ...
                <v-card-text>
                  <div class="text--primary">
                    <b>Автор(ы):</b> {{ this.book.author }}<br>
                    ...
                    <ul>
                      <li v-for="book_instance in this.book.book_instances" v-bind:key="book_instance"
                        ...
                        <v-card
                          ...
                          <v-spacer></v-spacer>
                          <v-btn block color="red" light @click="onReturnBook(book_instance)">Вернуть книгу в
                            библиотеку
                          </v-btn>
                        </v-card>
                        <br>
...
export default {
  ...
  async created() {
    [this.book, this.takenBooks] = await Promise.all([
      libraryApi.getBook(this.$route.params.id).then(result => result.data),
      libraryApi.getTakingBooks().then(result => result.data)
    ])
    ...
    for (const tb of this.takenBooks) {
      if (this.bookInstancesById[tb.book_instance]) {
        this.bookInstancesById[tb.book_instance].takenId = tb.reader
      }
    }
  },
  methods: {
    async onReturnBook(book_instance) {
      await libraryApi.deleteTakingBooks(book_instance.takenId)
      book_instance.takenId = null
    },
    async onSelectReader(bookId) {
      if (!this.readers) {
        const response = await libraryApi.getReadersList().then(res => res)
    ...
    async onGivingBook() {
      ...
      const response = await libraryApi.sendTakingBook({
