...
Суть сайта это подбор и манипулирование с рецептами для готовки, а именно с инструментами, ингридеентами и самими рецептами.
...
Это инструмент в разработке, который помогает переводить данные из формата Python в формат, подходящий для передачи через сеть или хранения в базе данных

У меня они написанны обычно, нечего сверъхестественного, выводя все поля и ставя вложенные сериализаторы для более логичной работы с апи
```python
...
## views.py
Здесь описана вся логика программы.

Тут Нужно было настроить самостоятельно update и create для рецепта и ингредиента, т.к. они используют встроенные сериализаторы
