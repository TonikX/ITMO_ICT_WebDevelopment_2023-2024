...
def jobRoom(request, pk):
    ...
    # messages.error(request, f'test:*{request.user}*')
    ...
    status = None
    if len(statuses) == 0:
        # messages.error(request, f'test: {2}')
        status = None
    elif 'Performer' in statuses:
        status = 'Performer'
    elif 'Сandidate' in statuses:
        # messages.error(request, f'test: {1}')

        status = 'Candidate'
    elif 'Participant' in statuses:
        status = 'Participant'

    # xmessages.error(request, f'statuses: {statuses}')
