...
server.run()
```
Написал код для клиентов и общего сервера. Запуск произвел через командную строку. Как видно из скрина, каждый клиент независимо добавляется в чат через отдельный поток.
Сервер слушает подключения новых клиентов


![задание №4](img/4_1.jpg)

Клиенты могут общаться друг с другом через общий интерфейс
