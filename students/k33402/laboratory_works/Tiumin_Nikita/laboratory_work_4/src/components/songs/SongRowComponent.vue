      ...
      <div class="last-listened__image_cover col-1 last-listened__image_container">
        <div style="margin:auto; position:relative">
          <div class="hover-content">
            <button aria-label="Play" class="__play_button">
              <play-icon @click="play" class="__play_icon"/>
            </button>
          </div>
...
const emit = defineEmits(['addedToFav'])
...
function addToFavorite() {
  http.post('songs/' + props.song.id + '/favorite').then(() => {
    emit('addedToFav')
  })
}
</script>
