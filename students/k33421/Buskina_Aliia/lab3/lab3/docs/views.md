...
class CleanerForGuestView(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Получите информацию о служащем, убиравшем номер указанного клиента в заданный день
        try:
            ...
            if cleaning_info:
                ...
                # Верните словарь с информацией о служащем (cleaner)
                return Response({
                    'cleaner': {
                        'name': cleaning_info.staff_id.name,
                        'surname': cleaning_info.staff_id.surname
                    },
                    'cleaning_info': serializer.data
                })
            else:
                return Response({'message': 'Cleaning information not found for the specified guest and date'})
