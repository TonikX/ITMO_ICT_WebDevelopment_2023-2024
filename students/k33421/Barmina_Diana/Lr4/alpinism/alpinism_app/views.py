...
class ParticipatingDetailView(RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        user_id = request.user.id#self.kwargs.get('user_pk')
        part = self.get_object()
        if part.user.id == user_id:
...
class ParticipatingDeleteView(generics.DestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        part = get_object_or_404(Participating, climbing_id=self.kwargs.get('pk'), alpinist_id=user_id)

        if part.alpinist_id.id == user_id:
            self.perform_destroy(part)
            return Response({'detail': 'Your participation successfully deleted'}, status=status.HTTP_200_OK)
        else:
            raise PermissionDenied(detail='You are nauthorized')
...
class ClubMembershipCreateView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if request.user.id == user_id:
            ...
            raise PermissionDenied(detail='wrong/Unauthorized')
...
class ClubMembershipDeleteView(generics.DestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        #part = self.get_object()
        user_id = request.user.id
        part = get_object_or_404(ClubMembership, club_id=self.kwargs.get('pk'), alpinist_id=user_id)
        if part.alpinist_id.id == user_id:#request.user.id == user_id:# and part.user.id == user_id:
            self.perform_destroy(part)
            return Response({'detail': 'Your club membership successfully deleted'}, status=status.HTTP_200_OK)
        else:
            raise PermissionDenied(detail=f'{user_id}')#'You are unauthorized')
